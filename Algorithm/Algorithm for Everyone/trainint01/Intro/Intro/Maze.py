...
def solve_maze(g, start, end):
    ...
    done = set()    # 기억 장소 2 : 이미 큐에 추가한 꼭짓점들을 잡합에 기록(중복 방지)
    
    qu.append(start)    # 출발점을 큐에 넣고 시작
    done.add(start)     # 집합에도 추가
    
    while qu:
        p = qu.pop(0)   # 큐에 처리해야 할 대상이 있으면 처리 대상을 꺼냄
        v = p[-1]       # 큐에 저장된 이동 경로의 마지막 문자가 현재 처리해야 할 꼭지점
        if v == end:
            return p    # 처리해야 할 꼭짓점이 도착점이라면 지금까지의 전체 이동 경로를 돌려주고 종료
        for x in g[v]:   # 대상 꼭짓점에 연결된 꼭짓점 중에
            if x not in done:   # 아직 큐에 추가된 적이 없는 꼭짓점을
                qu.append(p + x)    # 이동 경로에 새 꼭짓점으로 추가하여 큐에 저장하고
                done.add(x)         # 집합에도 추가함
                
    return "?"  # 탐색을 마칠 때까지 도착점이 나오지 않으면 나갈 수 없는 미로
